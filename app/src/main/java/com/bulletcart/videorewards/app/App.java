package com.bulletcart.videorewards.app;
...
import com.bulletcart.videorewards.BuildConfig;
...
import com.bulletcart.videorewards.Activities.LoginActivity;
...
import com.bulletcart.videorewards.Utils.LruBitmapCache;
...
//                                    Intent i = new Intent(getApplicationContext(), LoginActivity.class);
    ...
    public Boolean authorize(JSONObject authObj) {
        ...
        try {
            ...
            if (configArray.length() > 0) {
                ...
                String config_ratio = configObj.getString("ADMIN_RATIO");
                config_ratio = config_ratio.equals("") ? "0" : config_ratio;

                if(!config_ratio.equals("0")){
                    GlobalVariables.POINT_CURRENCY_RATIO = String.valueOf(GlobalFunctions.roundToDecimal(1 / Float.parseFloat(config_ratio), 6));
                }
                appStorage.store("RATIO", GlobalVariables.POINT_CURRENCY_RATIO);
    ...
    public void setUsedPoints(int points) {
        appStorage.store("used_points", points);
    }

    public int getUsedPoints() {
        return appStorage.get("used_points",0);
    }


    public String getRatio() {

        return appStorage.get("RATIO","0");
    }

    public Map<String, String> getCredential(){
        Map<String,String> params = new HashMap<>();
        params.put("clientId", GlobalConstants.CLIENT_ID);
        params.put("accountId", Long.toString(this.getId()));
        params.put("accessToken", this.getAccessToken());
        return params;
    }
    ...
    public String getData() {
        ...
        try{
            Jsonroot.put("clientId", GlobalConstants.CLIENT_ID);
    ...
    public void removeData() {
        ...
        appStorage.store("balance", "0");
        appStorage.store("RATIO", "0");
        ...
        appStorage.store("COMPANY_NAME", "");
        appStorage.store("COMPANY_SITE", "");
        appStorage.store("DAILY_REWARD", "");
        appStorage.store("REFER_REWARD", "");
        appStorage.store("PACKAGE_NAME", "");
        appStorage.store("REFER_ACTIVE", false);
        appStorage.store("DAILY_ACTIVE", false);
        appStorage.store("SUPPORT_EMAIL", "");
        appStorage.store("POLICY_ACTIVE", false);
        appStorage.store("APP_POLICY_URL", "");
        appStorage.store("APP_PREFIX", "");
        appStorage.store("APP_DEBUG_MODE",false);
        appStorage.store("APP_LICENSE_URL", "");
        appStorage.store("WEB_LICENSE_URL", "");
        appStorage.store("APP_TABS_ENABLE", false);
        appStorage.store("RATE_APP_ACTIVE", toString());
        appStorage.store("SHARE_APP_ACTIVE", false);
        appStorage.store("CONTACT_US_ACTIVE", false);
        appStorage.store("APP_NAVBAR_ENABLE", true);
        appStorage.store("APP_CONTACT_US_URL", "");
        appStorage.store("VENDOR_SUPPORT_URL", "");
        appStorage.store("INSTRUCTIONS_ACTIVE", false);
